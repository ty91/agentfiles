...
- `air`: MacBook Air for interactive development and mobile use. Tailscale DNS: `air.tail5f617e.ts.net`.
- `mini`: Mac mini server hosting personal agents and asynchronous development environments. Tailscale DNS: `mini.tail5f617e.ts.net`.
...
- Use `tailscale status --json` to check the named target's current IP and online state. Find its SSH alias and user in local SSH configuration; these SSH details are not yet recorded here. Ask for missing details only when needed for the task. Do not guess them or treat an online peer as sufficient identification.
