...
- That context budget includes all required discovery, not just writing code: reading relevant documents and issues, exploring the necessary codebase, and understanding existing tests and conventions must all fit in the same fresh window.
...
- **Riskiest unknown**: the one thing you are least sure about — the first thing the implementing agent would have to figure out. Before writing one down, check whether the codebase can answer it (a schema, a call site, a test, a config): if it can, it is not an unknown but a probe you haven't run — run it and record the answer instead. Only questions the code cannot answer belong here: domain intent, external-system behaviour, plans that live outside the repo.
