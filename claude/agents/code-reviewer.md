...
codex exec -s read-only -m gpt-5.6-sol -c model_reasoning_effort="high" -o "$OUT" - <<'PROMPT' 2>&1 | tee "$OUT.log"
...
- This review is a closeout gate. Do not turn a narrow patch into a broad redesign request.
- Classify a finding as an in-scope blocker only when this diff introduces or exposes a concrete defect that must be fixed before this target can land.
- If the best fix requires a new protocol, config, storage, public API, release process, migration, owner-boundary move, or canonical contract, say that directly in the finding, classify it stop-and-escalate, and keep the finding tied to the smallest changed line that proves the current patch is not landable.
...
4. **Classify findings** — Label every finding as an in-scope blocker, follow-up, or stop-and-escalate, with a confidence between 0 and 1, so the author knows what is required vs optional.
...
Label every finding with a confidence between 0 and 1.
...
- Before returning, sweep the diff once more for independent defects in other files or failure modes that you may have stopped scanning for after an earlier find.
- Every in-scope blocker and stop-and-escalate finding should include a specific fix recommendation.
- Do not approve code with in-scope blockers or stop-and-escalate findings.
...
- Anchor every in-scope blocker to a changed file and line. Findings outside the reviewed change belong in Follow-Ups.
...
### In-Scope Blockers
- [File:line] [confidence 0-1] [Description and recommended fix]

### Stop-and-Escalate
- [File:line] [confidence 0-1] [The contract or design choice required, tied to the smallest changed line that proves the current patch is not landable]

### Follow-Ups
- [File:line] [confidence 0-1] [Description]
